...
DuckDB as a SQL engine a MeTTa head can be registered into.
...
without it, or through `import metta_duckdb`, whose module body registers the same
rows.

```sh
pip install metta-duckdb          # or: pip install 'pymetta[sql]'
```

Its tests are in `tests/`, and they run in the workspace suite:

```sh
sh extensions/python/test.sh ext/metta-duckdb
```
